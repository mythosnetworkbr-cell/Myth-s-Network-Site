...
export async function openTicket(interaction: ChatInputCommandInteraction) {
  ...
  const existing = guild.channels.cache.find(
    channel => channel.type === ChannelType.GuildText && channel.name === `ticket-${interaction.user.id}`,
  );
...
export async function closeTicket(interaction: ChatInputCommandInteraction) {
  ...
  setTimeout(() => channel.delete('Ticket encerrado pelo usuário').catch(() => undefined), 5000);
}
