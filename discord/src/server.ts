...
function ipBytes(host: string): Buffer {
  ...
  if (parts.length !== 4 || parts.some(n => !Number.isInteger(n) || n < 0 || n > 255)) {
    throw new Error('SAMP_HOST deve ser um IPv4.');
  }
...
export async function querySamp(host: string, port: number, timeoutMs = 2500): Promise<ServerInfo> {
  ...
  return await new Promise(resolve => {
    ...
    socket.once('message', data => {
      try {
        if (data.length < 15 || data.subarray(0, 4).toString() !== 'SAMP' || data[10] !== 105) {
          return finish({ ...base, error: 'Resposta SAMP inválida.' });
        }
        let offset = 11;
        ...
        finish({ online: true, host, port, players, maxPlayers, hostname, gamemode, language, latencyMs: Date.now() - started, error: password ? 'Servidor protegido por senha.' : undefined });
      } catch {
        finish({ ...base, error: 'Resposta SAMP incompleta.' });
      }
    });
    socket.send(packet, port, host, error => {
      if (error) finish({ ...base, error: 'Não foi possível enviar a consulta UDP.' });
    });
  });
}
