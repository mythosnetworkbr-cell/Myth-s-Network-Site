...
export function currentUser(req:any,db:DB){const h=String(req.headers.authorization||'');const id=verifySession(h.replace(/^Bearer\s+/i,''));const u=id?db.users.find(u=>u.id===id)||null:null;if(!u)return null;return OWNER_EMAILS.includes(String(u.email).toLowerCase())?{...u,role:'owner'}:u;}
