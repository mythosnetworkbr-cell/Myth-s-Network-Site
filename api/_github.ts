...
export const OWNER_EMAILS=['quenidyyonline.17@gmail.com','gamesfloriano8@gmail.com'];
export const isOwnerEmail=(email:string)=>OWNER_EMAILS.includes(String(email||'').trim().toLowerCase());
...
export function addLog(db:DB,entry:any){db.logs=db.logs||[];db.logs.unshift({id:crypto.randomUUID(),created_at:new Date().toISOString(),...entry});if(db.logs.length>500)db.logs.length=500;}
...
export function currentUser(req:any,db:DB){const h=String(req.headers.authorization||'');const id=verifySession(h.replace(/^Bearer\s+/i,''));const u=id?db.users.find(u=>u.id===id)||null:null;if(!u)return null;return isOwnerEmail(u.email)?{...u,role:'owner'}:u;}
