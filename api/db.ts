...
const ROLES=['player','moderator','support','admin','developer'];
const staff=(u:any)=>!!u&&['admin','support'].includes(u.role);
const admin=(u:any)=>!!u&&(u.email===MASTER||u.role==='admin');
...
if(req.method==='GET'){
 ...
 if(table==='users'){if(!admin(u))return res.status(403).json({error:'Acesso negado'});return res.json({users:db.users.map(publicUser),count:db.users.length});}
 if(table==='logs'){if(!admin(u))return res.status(403).json({error:'Acesso negado'});return res.json({logs:(db.logs||[]).slice(0,500)});}
