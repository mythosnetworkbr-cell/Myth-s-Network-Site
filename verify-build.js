...
const ruleCards=(rulesIndex.match(/class="card"/g)||[]).length;
if(ruleCards<3)throw new Error(`Central de regras incompleta: apenas ${ruleCards} categoria(s) gerada(s)`);
const titles=[...rulesIndex.matchAll(/<a\b[^>]*class="card"[^>]*>[\s\S]*?<strong>([\s\S]*?)<\/strong>/gi)].map(m=>m[1].replace(/<[^>]+>/g,'').replace(/&amp;/g,'&').trim().toLocaleLowerCase('pt-BR'));
if(new Set(titles).size!==titles.length)throw new Error('Central de regras contém categorias duplicadas');
...
const ruleTitles=['BÍBLIA RP','CÓDIGOS Q','ALFABETO FONÉTICO','HOSPITAL'];
for(const term of ruleTitles){if(!new RegExp(term,'i').test(rulesIndex))throw new Error(`Categoria obrigatória ausente na Central: ${term}`)}
...
console.log(`Mythøs build verification OK — ${html.length} páginas HTML, ${ruleCards} categorias únicas, Bíblia/Q/Alfabeto/Hospital validados, acesso público validado e Polícia com ${policeItems} itens.`);
