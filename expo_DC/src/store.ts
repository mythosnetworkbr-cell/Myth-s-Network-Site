...
async function read(): Promise<Data> { try { return JSON.parse(await fs.readFile(file, 'utf8')) as Data; } catch { await fs.writeFile(file, JSON.stringify(initial, null, 2)); return structuredClone(initial); } }
