...
s=s.replace("useEffect(()=>{if(page==='staff'&&isAdmin)listUsers().then(setUsers)},[page,isAdmin]);", "useEffect(()=>{if(page==='staff'&&isAdmin)listUsers().then(setUsers)},[page,isAdmin]);\n useEffect(()=>{if(session?.user?.id)getProfileAvatar(session.user.id).then(setAvatar)},[session?.user?.id]);")
...
s=s.replace(old,new)
s=s.replace("{page==='new'&&<NewTicket category={category}setCategory={setCategory}subject={subject}setSubject={setSubject}message={message}setMessage={setMessage}createTicket={createTicket}/>} ", "{page==='new'&&<NewTicket category={category}setCategory={setCategory}subject={subject}setSubject={setSubject}message={message}setMessage={setMessage}createTicket={createTicket}/>} {page==='profile'&&<Profile session={session}avatar={avatar}setAvatar={setAvatar}displayName={displayName}/>} ")
...
# Replace the auth hero branding with the requested icon-only mark.
...
# Drawer: icon-only branding + profile entry.
...
# Staff: explicit registration count card.
...
# Add the profile page before Drawer.
